      ...
      <div className="bg-white dark:bg-[#1C2128] border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto -mx-4 sm:mx-0">
          <table className="w-full min-w-[600px] sm:min-w-0 text-left text-xs">
            <thead><tr className="border-b border-gray-100 dark:border-gray-800">{['ID','Type','Amount','Fee','Net','Status','Date','Actions'].map(h=><th key={h} className="px-5 py-3 whitespace-nowrap whitespace-nowrap text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{h}</th>)}</tr></thead>
            <tbody>
              {txs.map(tx=><tr key={tx.id} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-[#161B22]">
                <td className="px-5 py-3 whitespace-nowrap whitespace-nowrap text-gray-400 font-mono text-[11px]">{tx.id.slice(0,8)}...</td>
                ...
                <td className="px-5 py-3 whitespace-nowrap whitespace-nowrap font-semibold font-mono text-gray-900 dark:text-white">{formatCurrency(tx.amount)}</td>
                <td className="px-5 py-3 whitespace-nowrap whitespace-nowrap text-gray-500">{formatCurrency(tx.fee)}</td>
                <td className="px-5 py-3 whitespace-nowrap whitespace-nowrap font-semibold text-gray-900 dark:text-white">{formatCurrency(tx.net_amount)}</td>
                ...
                <td className="px-5 py-3 whitespace-nowrap whitespace-nowrap text-gray-400 text-[11px]">{new Date(tx.created_at).toLocaleDateString()}</td>
