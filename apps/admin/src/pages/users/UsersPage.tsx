    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        <div className="flex gap-2">
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search users…" className="px-3 py-2 whitespace-nowrap whitespace-nowrap bg-white dark:bg-[#161B22] border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-900 dark:text-white outline-none focus:border-blue-500 w-48 font-sans" />
          <button onClick={()=>setShowCreate(true)} className="px-4 py-2 whitespace-nowrap whitespace-nowrap bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold transition-all">+ New User</button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#1C2128] border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto -mx-4 sm:mx-0">
          <table className="w-full min-w-[600px] sm:min-w-0 text-left text-xs">
            <thead><tr className="border-b border-gray-100 dark:border-gray-800">{['User','Role','Status','AI','Joined','Actions'].map(h=><th key={h} className="px-5 py-3 whitespace-nowrap whitespace-nowrap text-[10px] font-semibold text-gray-400 uppercase tracking-wider">{h}</th>)}</tr></thead>
            <tbody>
              {users.map(u=><tr key={u.id} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-[#161B22]">
                <td className="px-5 py-3 whitespace-nowrap"><div className="font-semibold text-gray-900 dark:text-white">{u.display_name||u.email.split('@')[0]}</div><div className="text-[11px] text-gray-400">{u.email}</div></td>
                <td className="px-5 py-3 whitespace-nowrap"><span className={`text-[10px] font-bold ${u.role==='admin'?'text-red-600':'text-blue-600'}`}>{u.role?.toUpperCase()}</span></td>
                <td className="px-5 py-3 whitespace-nowrap"><span className={`text-[10px] font-bold px-2 py-0.5 whitespace-nowrap rounded ${u.is_active?'bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400':'bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400'}`}>{u.is_active?'ACTIVE':'SUSPENDED'}</span></td>
                <td className="px-5 py-3 whitespace-nowrap"><span className={`text-[10px] font-bold px-2 py-0.5 whitespace-nowrap rounded ${u.ai_enabled?'bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400':'bg-gray-100 dark:bg-gray-800 text-gray-500'}`}>{u.ai_enabled?'ON':'OFF'}</span></td>
                <td className="px-5 py-3 whitespace-nowrap whitespace-nowrap text-gray-400 text-[11px]">{formatDateTime(u.created_at)}</td>
