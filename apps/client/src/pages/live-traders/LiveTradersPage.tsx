  ...
  return (<>
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 px-4 py-2 whitespace-nowrap bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-full w-fit">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#1C2128] border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
          <div className="px-5 py-4 whitespace-nowrap border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-[#161B22]"><h3 className="text-sm font-semibold text-gray-900 dark:text-white">Leaderboard</h3></div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead><tr className="border-b border-gray-100 dark:border-gray-800">{['#','Trader','Win Rate','Trades','Profit','Streak'].map(h=><th key={h} className="px-5 py-3 whitespace-nowrap text-[10px] font-semibold text-gray-400 uppercase tracking-wider">{h}</th>)}</tr></thead>
              <tbody>
                {entries.map((e,i)=><tr key={e.id} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-[#161B22]">
                  <td className="px-5 py-3 whitespace-nowrap"><span className={`inline-flex w-6 h-6 rounded-full items-center justify-center text-[11px] font-bold ${i<3?'bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400':'bg-gray-100 dark:bg-gray-800 text-gray-500'}`}>{i+1}</span></td>
                  <td className="px-5 py-3 whitespace-nowrap font-semibold text-purple-600 dark:text-purple-400">{e.trader_id}</td>
                  <td className={`px-5 py-3 whitespace-nowrap font-semibold ${Number(e.win_rate)>=50?'text-green-600':'text-red-600'}`}>{formatPercent(e.win_rate,1)}</td>
                  <td className="px-5 py-3 whitespace-nowrap text-gray-600 dark:text-gray-400">{e.total_trades}</td>
                  <td className={`px-5 py-3 whitespace-nowrap font-semibold font-mono ${Number(e.total_profit)>=0?'text-green-600':'text-red-600'}`}>{Number(e.total_profit)>=0?'+':''}{formatCurrency(e.total_profit)}</td>
                  <td className={`px-5 py-3 whitespace-nowrap font-semibold ${e.current_streak>0?'text-green-600':e.current_streak<0?'text-red-600':'text-gray-400'}`}>{e.current_streak>0?`🔥 ${e.current_streak}`:e.current_streak<0?`❄️ ${Math.abs(e.current_streak)}`:'—'}</td>
                </tr>)}
              </tbody>
            </table>
        ...
        <div className="bg-white dark:bg-[#1C2128] border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm flex flex-col">
          <div className="px-5 py-4 whitespace-nowrap border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-[#161B22]"><h3 className="text-sm font-semibold text-gray-900 dark:text-white">Live Activity</h3></div>
          <div className="flex-1 overflow-y-auto max-h-[500px] divide-y divide-gray-50 dark:divide-gray-800/50">
            {(acts||[]).map((a:TraderActivity)=><div key={a.id} className="flex items-center gap-3 px-5 py-3 whitespace-nowrap">
